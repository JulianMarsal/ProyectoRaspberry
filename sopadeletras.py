...
def agregar_botones(layout,lista_pal,colores,ayuda, dic_pal, palabras,cantidad):
    ...
    if (ayuda=='Ambas' or ayuda=='Definición'):
            lis_definiciones=[]
            for x in dic_pal:
                    for palabra in dic_pal[x]:
                            definicion=palabras[0][x][palabra.lower()]
                            lis_definiciones.append(definicion)
                            lis_definiciones.append('\n')
            layout.append([sg.Text('Definiciones:')])
            layout.append([sg.Multiline(lis_definiciones)])
...
def crear_sopa(palabras_todas, config):
    ...
    return agregar_botones(layout,lista_palabras,config[0]['colores'],config[0]['ayuda'], diccionario_palabras, palabras_todas,config[0]['cant_palabras']),coord_palabras
...
def Main():
    ...
    layout,coor=crear_sopa(palabras,config)
