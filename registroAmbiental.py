...
def guardar(info, oficina):
	archivo= open('json files/datos-oficinas.json','r')
	json_datos=json.load(archivo)
	archivo.close
	json_datos[oficina].append(info)
	objeto=json.dumps(json_datos,indent=4)
	archivo=open('json files/datos-oficinas.json','w')
...
def opciones():
    ...
    while True:
        ...
        if event == None or event=='Aceptar':
            ...
            datos[nueva_oficina]=[]
            objeto=json.dumps(datos,indent=4)
            archivo=open('json files/datos-oficinas.json','w')
            archivo.write(objeto)	
            archivo.close
