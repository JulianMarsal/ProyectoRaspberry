...
def actualizar_archivoPal(objeto):
    ...
    datos_archivo=leer_archivo('palabras.json')
    for x in objeto[0]:
...
def eliminar(palabra):
    ...
    datos=leer_archivo('palabras.json')
    for x in datos[0]:
    ...
    archivo.write(nuevos)
    archivo.close
    layout=[[sg.Text('Se ha eliminado con exito',font='Courier')],
            [sg.Button('Aceptar')]]
