...
import random as rd
import string
from configuracion import leer_archivo
...
def mostrar_def(palabras_todas,palabras_sopa):
    ...
    lis_definiciones=('-------').join(lis_definiciones)
    for elem in caracteres:
         lis_definiciones=lis_definiciones.strip(elem)   
    layout2=[[sg.Multiline(lis_definiciones, size=(35,3))]]
    window2 = sg.Window('Definiciones').Layout(layout2)
    event,valores=window2.Read()
    window2.Close()

def Main():
    colores=leer_archivo('colores.json')
    ...
    while volver_a_jugar:
        ...
        while True:
            ...
            if(event is None or event=='Volver al menú'):
                ...
                logrado=comprobar(ingresados, coor)
                if logrado:
                    layout2=[[sg.Text('¿Quieres volver a jugar?',font='Courier')],
